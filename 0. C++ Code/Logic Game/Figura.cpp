...
void Figura::dibuixaFigura() const //Funció per dibuixar una figura.
{ 	
		...
		for (int j = 0; j < m_amplada; j++)
		{
			if (m_figura[i][j] != NO_COLOR) //Per totes les posicions de la figura que siguin de colors.
			{
				GraphicManager::getInstance()->drawSprite((IMAGE_NAME)(m_figura[i][j] + 1), //Color de la figura.
					POS_X_TAULER + ((m_x + j) * MIDA_QUADRAT), POS_Y_TAULER + ((m_y + i) * MIDA_QUADRAT), false); //Posició x i posició y.
			}
		}
	}
}

int Figura::Aleatori(int NumMax) //Funció per generar un número aleatori.
{
	int NumAleatori;
	
	if (NumMax != 0)
		NumAleatori = rand() % NumMax + 1; //Generem un número aleatori per determinar la figura següent.
	else
		NumAleatori = 2 + rand() % 7; //Generem un número aleatori per determinar lo posició x en la que es generarà la figura.

	return NumAleatori; //Retornem el número aleatori.
}
...
void Figura::figuraAleatoria()
{
	...
	{
		girar(GIR_HORARI); //Girem la figura aleatori un número aleatori de vegades.
	}
}
...
void Figura::dibuixaSeguent() const
{
	for (int i = 0; i < m_alcada; i++) //Recorrem cada posició que ocupa la figura i la pintem amb el seu color corresponent a la posició corresponent.
	{
		for (int j = 0; j < m_amplada; j++)
		{
			...
			{
				GraphicManager::getInstance()->drawSprite((IMAGE_NAME)(m_figura[i][j] + 1), //Color de la figura.
					POS_X_TAULER + ((15 + j) * MIDA_QUADRAT), POS_Y_TAULER + ((i) * MIDA_QUADRAT), false); //Posició x i posició y.
			}
		}
	}
}
...
void Figura::dibuixaSeguent2() const
{
	for (int i = 0; i < m_alcada; i++) //Recorrem cada posició que ocupa la figura i la pintem amb el seu color corresponent a la posició corresponent.
	{
		for (int j = 0; j < m_amplada; j++)
		{
			...
			{
				GraphicManager::getInstance()->drawSprite((IMAGE_NAME)(m_figura[i][j] + 1), //Color de la figura.
					POS_X_TAULER + ((15 + j) * MIDA_QUADRAT), POS_Y_TAULER + ((5 + i) * MIDA_QUADRAT), false); //Posició x i posició y.
			}
		}
	}
}
...
void Figura::dibuixaSeguent3() const
{
	for (int i = 0; i < m_alcada; i++) //Recorrem cada posició que ocupa la figura i la pintem amb el seu color corresponent a la posició corresponent.
	{
		for (int j = 0; j < m_amplada; j++)
		{
			...
			{
				GraphicManager::getInstance()->drawSprite((IMAGE_NAME)(m_figura[i][j] + 1), //Color de la figura.
					POS_X_TAULER + ((15 + j) * MIDA_QUADRAT), POS_Y_TAULER + ((10 + i) * MIDA_QUADRAT), false); //Posició x i posició y.
			}
		}
	}
}
